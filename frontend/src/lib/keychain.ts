...
export async function hasEOA(): Promise<boolean> {
    return !!(await idbGet<string>(KEY_EOA_PRIV));
}
...
export async function ensureEOA(): Promise<Wallet> {
    ...
    if (existing && /^0x[0-9a-fA-F]{64}$/.test(existing)) {
        return new Wallet(existing);              // Wallet ✅
    }
    const hd = Wallet.createRandom();           // HDNodeWallet
    await idbSet(KEY_EOA_PRIV, hd.privateKey);
    return new Wallet(hd.privateKey);           // конвертируем в Wallet ✅
}
...
export async function hasRSA(): Promise<boolean> {
    return !!(await idbGet<ArrayBuffer>(KEY_RSA_PRIV_PKCS8));
}
...
export async function ensureRSA(): Promise<{ privateKey: CryptoKey; publicPem: string }> {
    ...
    if (existing) {
        ...
        const storedPubPem = await idbGet<string>(KEY_RSA_PUB_PEM);
        const pubPem = storedPubPem ?? (await exportRsaPublicPemFromPrivate(priv));
        if (!storedPubPem) await idbSet(KEY_RSA_PUB_PEM, pubPem);
...
async function exportRsaPublicPemFromPrivate(priv: CryptoKey): Promise<string> {
    ...
    const pub = await crypto.subtle.importKey(
        "jwk",
        {kty: "RSA", n: jwk.n, e: jwk.e, alg: "PS256", ext: true},
        {name: "RSA-PSS", hash: "SHA-256"},
        true,
        ["verify"]
    );
...
export async function restoreFromBackup(file: File, password: string): Promise<{ address: string }> {
    ...
    const j = JSON.parse(txt) as {
        version: number;
        kdf?: { iterations?: number; salt_b64?: string };
        cipher?: { iv_b64?: string; data_b64?: string };
    };
    ...
    const priv = await crypto.subtle.importKey(
        "pkcs8",
        b64toAb(payload.rsaPrivPkcs8_b64),
        {name: "RSA-PSS", hash: "SHA-256"},
        true,
        ["sign"]
    );
...
/** ensureEOA() должен вернуть Wallet из локального приватника */
export async function signLoginTyped(message: LoginMessage): Promise<string> {
