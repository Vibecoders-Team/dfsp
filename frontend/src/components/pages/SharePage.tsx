...
import { getAgent, getSelectedAgentKind } from '@/lib/agent/manager.ts';
...
export default function SharePage() {
  ...
  const [addressMismatch, setAddressMismatch] = useState<{ expected: string; actual: string } | null>(null);
  ...
  async function performShare() {
    try {
      ...
      // TON-only users cannot perform on-chain share operations
      if (user?.authMethod === 'ton') {
        setError(
          'On-chain sharing requires an EVM wallet. ' +
          'TON signatures cannot be verified by Ethereum smart contracts. ' +
          'Please link MetaMask or WalletConnect from the Login page to use share features.'
        );
      ...
      if (isLocal) {
        ...
        setNetworkHint('');
        // Check if agent address matches logged-in user address
        // This prevents signing grants with wrong wallet
        if (user?.address) {
          try {
            const agentAddr = await agent.getAddress();
            if (agentAddr.toLowerCase() !== user.address.toLowerCase()) {
              setAddressMismatch({ expected: user.address, actual: agentAddr });
              setError(`Wallet address mismatch: logged in as ${user.address.slice(0,6)}...${user.address.slice(-4)}, but ${getSelectedAgentKind()} wallet is ${agentAddr.slice(0,6)}...${agentAddr.slice(-4)}. Please switch to the correct wallet or re-login with this wallet.`);
              setIsSubmitting(false);
              return;
            }
          } catch (e) {
            // If we can't get address, continue (might be connection issue)
  ...
  const retryMetaTx = async () => {
    ...
    try {
      ...
      if (!isLocal && user?.address) {
        try {
          const agentAddr = await agent.getAddress();
          if (agentAddr.toLowerCase() !== user.address.toLowerCase()) {
            setAddressMismatch({ expected: user.address, actual: agentAddr });
            toast.error(`Wallet address mismatch. Please switch to ${user.address.slice(0,6)}...${user.address.slice(-4)} or re-login.`);
            return;
          }
        } catch (e) {
          console.warn('Could not verify agent address:', e);
      ...
      if (isLocal) {
        ...
        if (user?.address) {
          try {
            ...
            if (agentAddr.toLowerCase() !== user.address.toLowerCase()) {
              setAddressMismatch({ expected: user.address, actual: agentAddr });
              throw new Error(`Wallet address mismatch. Please switch to ${user.address.slice(0,6)}...${user.address.slice(-4)} or re-login.`);
            }
          } catch (e) {
         ...
         {addressMismatch && (
           <Alert variant="destructive">
             ...
             <AlertDescription>
               <div className="space-y-2">
                 <div>
                   <strong>Wallet address mismatch!</strong><br/>
                   You are logged in as <code className="bg-muted px-1 rounded">{addressMismatch.expected.slice(0,6)}...{addressMismatch.expected.slice(-4)}</code>,
                   but your current {getSelectedAgentKind()} wallet is <code className="bg-muted px-1 rounded">{addressMismatch.actual.slice(0,6)}...{addressMismatch.actual.slice(-4)}</code>.
                 </div>
                 <div className="text-xs">
                   To fix this, either:
                   <ul className="list-disc ml-4 mt-1">
                     <li>Switch your {getSelectedAgentKind()} wallet to the address you logged in with</li>
                     <li>Or log out and log in again with your current wallet</li>
                     <li>Or switch to Local signer if you registered with a local wallet</li>
                   </ul>
                 </div>
               </div>
             </AlertDescription>
