...
// безопасно достаём checksum из произвольного объекта
function readChecksum(v: unknown): string | undefined {
    if (typeof v === "object" && v !== null) {
        const c = (v as Record<string, unknown>).checksum;
...
export default function Verify() {
    ...
    async function onVerify() {
        ...
        try {
            ...
            const onchain = readChecksum(m) ?? "";
            setMatch(onchain.toLowerCase() === s2.toLowerCase());
        } catch (e: unknown) {
            setErr(getErrorMessage(e, "Verify failed"));
        }
    }
    ...
    return (
        <div style={{maxWidth: 900, margin: "2rem auto", fontFamily: "Inter, system-ui"}}>
            ...
            {meta !== null && (<><h3>On-chain</h3>
                <pre>{JSON.stringify(meta, null, 2)}</pre>
            </>)}
            {(sha256 || keccak) && (<><h3>Local</h3>
                <pre>{JSON.stringify({sha256, keccak}, null, 2)}</pre>
            </>)}
            {match !== null && (
                <p style={{fontWeight: 600, color: match ? "green" : "crimson"}}>
