...
describe("AccessControlDFSP - extended", () => {
  ...
  it("grant with grantee=0x0 reverts InvalidGrantee", async () => {
    ...
    await expect(
      ac.connect(grantor).grant(fid, ethers.ZeroAddress, 3600, 1)
    ).to.be.revertedWithCustomError(ac, "InvalidGrantee");
  });
  ...
  it("_msgSender works for grant/useOnce/revoke and direct calls also succeed", async () => {
    const [fwdOwner, grantor, grantee] = await ethers.getSigners();
    ...
    // Direct grant
    const capDirect = await ac.connect(grantor).grant.staticCall(
      fid,
      await grantee.getAddress(),
      3600,
      3
    ...
    // Meta grant
    const dataGrant = ac.interface.encodeFunctionData("grant", [
      fid,
      await grantee.getAddress(),
      3600,
      3,
    ...
    // Deterministic meta grant
    const fid2 = ethers.id("meta-2");
    const n2 = await ac.grantNonces(await grantor.getAddress());
    const expectedCap2 = keccakAbiEncode(
      await grantor.getAddress(),
      await grantee.getAddress(),
      fid2,
      n2
    );
    const dataGrant2 = ac.interface.encodeFunctionData("grant", [
      fid2,
      await grantee.getAddress(),
      3600,
      1,
    ...
    await signAndExecute(
      ...
      grantee,
      await ac.getAddress(),
      ac.interface.encodeFunctionData("useOnce", [expectedCap2])
    );
    ...
    await signAndExecute(
      ...
      grantor,
      await ac.getAddress(),
      ac.interface.encodeFunctionData("revoke", [expectedCap2])
    );
  ...
  it("deterministic capId off-chain vs on-chain (direct grant)", async () => {
    ...
    const expected = keccakAbiEncode(
      await grantor.getAddress(),
      await grantee.getAddress(),
      fid,
      n
    ...
    const capStatic = await ac
      .connect(grantor)
      .grant.staticCall(fid, await grantee.getAddress(), 100, 1);
