...
log "Pre-building compose images"; docker compose -f "$COMPOSE_FILE" --env-file "$ENV_FILE" build api celery-worker celery-beat contracts explorer-indexer explorer-web || warn "Compose build issues"
...
log "Starting api + celery"; docker compose -f "$COMPOSE_FILE" --env-file "$ENV_FILE" up -d api celery-worker celery-beat || warn "app layer issues"; sleep 5
